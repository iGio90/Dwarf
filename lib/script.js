...
function DwarfApi() {
    ...
    this.hookJavaConstructor = function(className, implementation, restore) {
        ...
        Java.perform(function () {
            try {
                var handler = Java.use(className);
                javaHelper.hook(handler, '$init', implementation, restore);
                handler.$dispose();
            } catch (err) {}
        });
    };
    ...
    this.hookJavaMethod = function(targetClassMethod, implementation, restore) {
        ...
        Java.perform(function () {
            ...
            var handler;
            try {
                handler = Java.use(targetClass);
            } catch (err) {
                return;
            }

            javaHelper.hook(handler, targetMethod, implementation, restore);
            handler.$dispose();
        });
    };
...
function JavaHelper() {
    ...
    this.hook = function(handler, method, implementation, restore) {
        ...
        if (overloadCount > 0) {
            ...
            for (var i = 0; i < overloadCount; i++) {
                var impl = null;
                if (!restore) {
                    var mArgs = hook[method].overloads[i].argumentTypes;
                    h.javaOverloads[mArgs.length] = mArgs;
                    impl = javaHelper.hookImplementation(method, implementation);
                }
                hook[method].overloads[i].implementation = impl;
            }
        }
    };

    this.hookImplementation = function(method, implementation) {
        var args = arguments;
        ...
        Java.perform(function() {
            classes.forEach(function(className) {
                try {
                    ...
                    if (overloadCount > 0) {
                        for (var i = 0; i < overloadCount; i++) {
                            clazz["$init"].overloads[i].implementation = function() {
                    ...
                    methods.forEach(function(method) {
                        ...
                        if (overloadCount > 0) {
                            for (var i = 0; i < overloadCount; i++) {
                                clazz[method].overloads[i].implementation = function() {
...
var ThreadWrapper = function() {
    ...
    this.new = function(fn) {
        ...
        this.handler_fn = fn;
        return this.pthread_create(pthread_t, ptr(0), this.handler, ptr(0));
    };
