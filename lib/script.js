...
var baseArch = Process.arch.indexOf('arm') >= 0 ? 'arm' : 'x86';
...
function DwarfApi() {
    ...
    this.getRange = function(pt) {
        try {
            ...
            return Process.findRangeByAddress(pt);
        } catch (e) {
            return []
        }
    };
...
function DwarfKernel() {
    ...
    this.execve = new NativeFunction(Module.findExportByName(libc, 'execve'), 'int', ['pointer', 'pointer', 'pointer']);
