...
function DwarfKernel() {
    ...
    function Ftrace(kernel) {
        ...
        this.readTrace = function() {
            var f = fs.open(kernel.ftrace.PATH_TRACE, 'r');
        ...
        this.setEvents = function(events) {
            if (this.kernel.root_pids.indexOf(Process.getCurrentThreadId()) < 0) {
                var err = this.kernel.root();
                if (err) {
                    console.log('setCurrentEvents err: ' + err);
                    return err;
                }
            }
        ...
        this.setTracing = function(tracing) {
            if (this.kernel.root_pids.indexOf(Process.getCurrentThreadId()) < 0) {
                var err = this.kernel.root();
                if (err) {
                    console.log('setTracing err: ' + err);
                    return err;
                }
            }
