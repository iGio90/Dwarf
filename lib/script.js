    ...
    Java.perform(function () {
        ...
        for (var i = 0; i < overloadCount; i++) {
            hook[targetMethod].overloads[i].implementation = function() {
                ...
                var newContext = onHook(targetClassMethod, args);
                if (newContext !== null) {
...
if (phdr_tgds_ptr > 0 && do_dlopen_ptr > 0) {
    ...
    Interceptor.replace(phdr_tgds_ptr, new NativeCallback(function (a, b, c, d, e) {
        if (hooked_onload !== null) {
            send("2:::" + hooked_onload['module'] + ':::' + c + ':::' + gettid());
            hooked_onload['context'] = this.context;
            var newContext = onHook(this.context.pc, this.context);
            if (newContext != null) {
        ...
        try {
            ...
            hooks[p] = {
                'i': Interceptor.attach(p, function () {
                    ...
                    var newContext = onHook(p, this.context);
                    if (newContext !== null) {
                        for (var k in this.context) {
