...
function Dwarf() {
    function _DwarfApi() {
        this._detach = function() {
            ...
            while (Object.keys(getDwarf().hook_contexts).length > 0) {
                Thread.sleep(1);
            }
        };
        ...
        this.javaExplorer = function(what) {
            if (typeof this['hook_context'] === 'undefined') {
                console.log('Explorer arg outside scope');
                return {};
            } else {
                var handle;
                ...
                var ol = Object.getOwnPropertyNames(handle.__proto__);
                var ret = {};
                for (var o in ol) {
                    var name = ol[o];
                    try {
                        ...
                        if (t === 'function') {
                            ...
                            if (typeof Object.getOwnPropertyNames(handle[name].__proto__)['$className'] !== 'undefined') {
                                value = handle[name]['$className']
                            } else if (typeof handle[name]['$handle'] !== 'undefined' &&
                                handle[name]['$handle'] !== null) {
                                value = handle[name]['$handle'];
                                sub_handle = handle[name]['$handle'];
                            } else if (typeof handle[name]['toString'] !== 'undefined') {
                                value = handle[name].toString();
                            } else {
                                console.log('Explorer met unknown argument');
                                console.log(Object.getOwnPropertyNames(handle[name].__proto__));
                                continue;
                            }
                        } else {
                            value = handle[name];
