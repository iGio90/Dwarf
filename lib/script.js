...
function JavaHelper() {
    ...
    this.hook = function(clazz, method, implementation, restore) {
        Java.perform(function () {
            var handler;
            try {
                handler = Java.use(clazz);
            } catch (err) {
                return;
            }
            ...
            var classMethod = clazz + '.' + method;

            if (overloadCount > 0) {
                var hook;
                if (!restore) {
                    send('hook_java_callback:::' + clazz + '.' + method);
                    hook = new Hook();
                ...
                for (var i = 0; i < overloadCount; i++) {
                    ...
                    if (!restore) {
                        ...
                        impl = javaHelper.hookImplementation(clazz, method, hook, implementation);
                    }
    ...
    this.hookImplementation = function(clazz, method, hook, implementation) {
        return function () {
            var classMethod = clazz + '.' + method;
            var args = arguments;
            ...
            if (javaHelper._traced_classes.indexOf(clazz) >= 0) {
                // call trace implementation
                javaHelper.traceImplementation(clazz, method).apply(this, arguments);
            }
        ...
        Java.perform(function() {
            classes.forEach(function(className) {
                try {
                    ...
                    var classMethod = clazz + '.$init';
                    if (typeof getDwarf().hooks[classMethod] === 'undefined') {
                        var overloadCount = clazz["$init"].overloads.length;
                        if (overloadCount > 0) {
                            for (var i = 0; i < overloadCount; i++) {
                                clazz[method].overloads[i].implementation = javaHelper.traceImplementation(clazz, method);
                            }
                        }
                    }
    ...
    this.traceImplementation = function(clazz, method) {
        return function() {
            send('java_trace:::enter:::' + clazz + '.' + method + ':::' + JSON.stringify(arguments));
            var ret = this[method].apply(this, arguments);
            send('java_trace:::leave:::' + clazz + '.' + method + ':::' + ret);
            return ret;
        }
    }
}
