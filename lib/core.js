...
function DwarfApi() {
    ...
    this.startNativeFunctionTracer = function (tid) {
        if (!isDefined(tid)) {
            tid = Process.getCurrentThreadId();
        }
        ...
        Stalker.follow(tid, {
            ...
            onReceive: function (events) {
                ...
                events.forEach(function (event) {
                    result.append({
                        type: event[0],
                        src: Instruction.parse(event[1]),
                        dest: Instruction.parse(event[2]),
                        depth: event[3]
                    })
                });

                send('native_function_tracer:::' + tid + JSON.stringify(result))
            }
        });
    ...
    this.stopNativeFunctionTracer = function (tid) {
        if (!isDefined(tid)) {
            tid = Process.getCurrentThreadId();
        }
