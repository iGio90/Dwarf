    ...
    this.initializeApi = function() {
        ...
        getDwarf()._stalkerCallout = function(context) {
            ...
            if (DEBUG) {
                ...
                _log('[' + tid + '] callOut: ' + 'running callout', insn.address, insn.toString());
            }
        ...
        getDwarf()._onHook = function (reason, p, context, hook, java_handle) {
            ...
            if (isDefined(getDwarf().tid_native_contexts[tid])) {
                delete getDwarf().tid_native_contexts[tid];
            }
            ...
            for (var s in getDwarf().nativeOnLoads) {
                if (moduleName.indexOf(s) >= 0) {
                    ...
                    if (typeof hook !== 'undefined') {
                        ...
                        getDwarf().tid_native_contexts[tid] = this.context;
...
function DwarfApi() {
    ...
    this.startNativeTracer = function (mode, tid) {
        if (!isDefined(tid)) {
            tid = Process.getCurrentThreadId();
        }

        if (isDefined(getDwarf().stalked_threads[tid])) {
            console.log('thread ' + tid + ' is already traced');
            return 0;
        }

        const modes = ['functions', 'instructions', 'blocks'];
        ...
        if (modes.indexOf(mode) < 0) {
            console.log('tracer mode not supported. Use on of: ' + JSON.stringify(modes));
            return 0;
        }

        getDwarf().stalked_threads[tid] = true;
        ...
        Stalker.follow(tid, {
            ...
            onReceive: function (events) {
                ...
                events.forEach(function (event) {
                    ...
                    if (mode === 'functions') {
                        obj.src = Instruction.parse(event[1]);
                        obj.dest = Instruction.parse(event[2]);
                        obj.depth = event[3];
                    } else if (mode === 'intructions') {
                        obj.src = Instruction.parse(event[1]);
                    } else if (mode === 'blocks') {
                        obj.start = Instruction.parse(event[1]);
                        obj.end = Instruction.parse(event[2]);
                    }

                    result.push(obj);
                });

                send('native_tracer:::' + tid + ':::' + JSON.stringify(result))
            }
        });
    ...
    this.step = function () {
        ...
        if (!isDefined(getDwarf().step_info[tid])) {
    ...
    this.stopNativeTracer = function (tid) {
        if (!isDefined(tid)) {
            tid = Process.getCurrentThreadId();
        }

        if (!isDefined(getDwarf().stalked_threads[tid])) {
            console.log(tid + ' is not currently traced');
            return 0;
        }
        ...
        try {
            ...
            if (typeof logic === 'function') {
                hook.interceptor = Interceptor.attach(hook.nativePtr, function (args) {
                    ...
                    getDwarf().tid_native_contexts[Process.getCurrentThreadId()] = this.context;
