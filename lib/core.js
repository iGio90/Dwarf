...
function Dwarf() {
    ...
    this._sendInfos = function (reason, p, ctx, hook) {
        ...
        if (ctx !== null) {
            ...
            data['modules'] = api.enumerateModules(true);
            data['pid'] = Process.id;
        ...
        if (Process.platform === 'windows') {
            ...
            if (javaHelper._sdk >= 23) {
                ...
                if (phdr_tgds_ptr > 0 && do_dlopen_ptr > 0) {
                    ...
                    Interceptor.attach(phdr_tgds_ptr,  {
                        onEnter: function (args) {
                            this.base = args[2]
                        },
                        onLeave: function (ret) {
                            if (hook !== null) {
                                loggedSend("native_on_load_callback:::" + hook.module + ':::' + this.base + ':::' + Process.getCurrentThreadId());
                                getDwarf()._onHook(REASON_HOOK, this.context.pc, this.context, hook, null);
                                hook = null;
                            }
                        }
                    ...
                    Interceptor.attach(do_dlopen_ptr, {
                        onEnter: function (args) {
                            this.module_name = Memory.readCString(args[0]);
                            for (var s in getDwarf().nativeOnLoads) {
                                if (this.module_name.indexOf(s) >= 0) {
                                    hook = getDwarf().nativeOnLoads[s];
                                }
                            }
                        },
                        onLeave: function () {
                            var module = api.findModule(this.module_name);
                            loggedSend('native_on_load_module_loading:::' + Process.getCurrentThreadId() + ':::' + JSON.stringify(module));
                        }
                    });
                }
            } else {
                                    ...
                                    for (var s in getDwarf().nativeOnLoads) {
                                        if (w.indexOf(s) >= 0) {
                                            ...
                                            if (typeof hook !== 'undefined') {
                                                ...
                                                loggedSend("native_on_load_callback:::" + hook.module + ':::' + JSON.stringify(m));
                                                getDwarf()._onHook(REASON_HOOK, this.context.pc, this.context, hook, null);
                                            }
                                        }
                                    }
...
function DwarfApi() {
    ...
    this.enumerateModules = function (sync) {
        sync = sync || false;
        var modules = {};
        if(sync) {
        ...
        for(var i = 0; i < modules.length; i++) {
            ...
            try {
                modules[i].imports = api.enumerateImports(modules[i].name);
                modules[i].exports = api.enumerateExports(modules[i].name);
                modules[i].symbols = api.enumerateSymbols(modules[i].name);
            } catch(e) {}
        }
        return modules;
    };
    ...
    this.findModule = function (module) {
        if (typeof module === 'string') {
            return Process.findModuleByName(module);
        } else {
            return Process.findModuleByAddress(ptr(module));
        }
    };
    ...
    this.updateModules = function () {
        var modules = api.enumerateModules(true);
