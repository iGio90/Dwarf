...
function Dwarf() {
    ...
    this.start = function () {
        ...
        var sleep_impl_ptr = api.findExport('sleep');
        if (parseInt(sleep_impl_ptr) === 0) {
            this.sleep_impl = Thread.sleep;
        } else {
            this.sleep_impl = new NativeFunction(sleep_impl_ptr, 'int', ['int'])
        }
    ...
    this.singleStep = function(tid) {
        ...
        if (typeof hc !== 'undefined') {
            ...
            if (hc.java_handle === null) {
                ...
                if (typeof hook === 'undefined') {
                    hook = new Hook();
                    hook.nativePtr = instruction.next;
                    _log('stepping to -> ' + dethumbedPtr);
                    var stepHook = Interceptor.attach(hook.nativePtr, function () {
                        _log('step hook?');
                    });
                }
