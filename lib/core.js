...
function Dwarf() {
    ...
    this._onHook = function (reason, p, context, hook, java_handle) {
        ...
        if (shouldSleep) {
            ...
            if (DEBUG) {
                _log('[' + hc.tid + '] sleeping context. goodnight!');
            }
    ...
    this._sendInfos = function (reason, p, ctx) {
        ...
        if (bt !== null) {
            loggedSend('backtrace:::' + JSON.stringify(bt));
        }
    };
    ...
    this.start = function () {
        ...
        this.sleep_impl = api.findExport('sleep');
        if (parseInt(this.sleep_impl) === 0) {
            this.sleep_impl = Thread.sleep;
        } else {
            this.sleep_impl = new NativeFunction(this.sleep_impl, 'int', ['int'])
        }
        ...
        if (Process.platform === 'windows') {
                    ...
                    try {
                        ...
                        for (var s in getDwarf().onLoads) {
                            if (w.indexOf(s) >= 0) {
                                ...
                                loggedSend("onload_callback:::" + hook.module + ':::' + 0 + ':::' + Process.getCurrentThreadId());
            ...
            if (phdr_tgds_ptr > 0 && do_dlopen_ptr > 0) {
                ...
                wrappedInterceptor.attach(phdr_tgds_ptr, function (args) {
                    if (hook !== null) {
                        loggedSend("onload_callback:::" + hook.module + ':::' + args[2] + ':::' + Process.getCurrentThreadId());
                        getDwarf()._onHook(REASON_HOOK, this.context.pc, this.context, hook, null);
                        hook = null;
                    }
                });
            ...
            // attach to Application.onCreate for init debugging
            javaHelper.hook('android.app.Application', 'onCreate', null, false, true);
        }
    };
}
...
function DwarfApi() {
    ...
    this.deleteHook = function(key) {
        ...
        if (hook.interceptor !== null) {
            ...
            loggedSend('hook_deleted:::onload:::' + hook.module);
        }
    };
    ...
    this.hookOnLoad = function(m, logic) {
        if (getDwarf().onLoads[m] === null || typeof(getDwarf().onLoads[m]) === 'undefined') {
            ...
            loggedSend('hook_onload_callback:::' + m);
        }
    };
    ...
    this.nativeBacktrace = function(ctx) {
        // frida doesn't love to make more than 1 backtrace. timeout occurs and proc is byebye
        while (this._backtracing) {
            getDwarf().sleep_impl(0.01);
        }

        this._backtracing = true;
        var _ret = Thread.backtrace(ctx, Backtracer.ACCURATE)
    ...
    this.singleStep = function(tid) {
        _log('step');
        var hc = getDwarf().hook_contexts[parseInt(tid)];
        _log(hc);
        if (typeof hc !== 'undefined') {
            // check if it's native
            if (hc.java_handle === null) {
                var instruction = Instruction.parse(hc.context[getDwarf().procedure_call_register]);

                _log(JSON.stringify(instruction));

                var dethumbedPtr = getDwarf()._dethumbify(instruction.next);
                var hook = getDwarf().hooks[dethumbedPtr];

                // skip if we already have an hook in next pointer
                if (typeof hook === 'undefined') {
                    hook = new Hook();
                    hook.nativePtr = dethumbedPtr;
                    _log('stepping to -> ' + dethumbedPtr);
                    wrappedInterceptor.attach(dethumbedPtr, function () {
                        getDwarf()._onHook(REASON_SINGLE_STEP, hook.nativePtr, this.context, hook, null);
                    });
                }
...
function JavaHelper() {
    ...
    this.hook = function(className, method, implementation, restore, internal) {
