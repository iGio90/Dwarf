...
function Dwarf() {
    ...
    this._onHook = function (reason, p, context, hook, java_handle) {
        ...
        if (shouldSleep) {
            ...
            if (DEBUG) {
                _log('[' + tid + '] sleeping context. goodnight!');
            }
...
function DwarfApi() {
    ...
    this.deleteHook = function(key) {
        ...
        if (hook.interceptor !== null) {
            ...
            delete getDwarf().hooks[key];
            send('hook_deleted:::java:::' + key);
        } else if (hook.module !== null) {
            delete getDwarf().onLoads[hook.module];
            send('hook_deleted:::onload:::' + hook.module);
        }
    };
    ...
    this.memoryScan = function(start, size, pattern) {
        ...
        send('memoryscan_result:::' + JSON.stringify(result));
    };
