...
function Dwarf() {
    ...
    this.start = function () {
        ...
        if (Process.platform === 'windows') {
            ...
            // attach to VMClassLoader to notify for new loaded class
            Java.performNow(function () {
                var handler = Java.use('java.lang.VMClassLoader');
                var overload = handler.findLoadedClass.overload('java.lang.ClassLoader', 'java.lang.String');
                overload.implementation = function(cloader, clazz) {
                    ...
                    return overload.call(this, cloader, clazz);
                };
            });
        }
    };
...
function DwarfApi() {
    ...
    this.release = function(tid) {
        ...
        if (typeof tid === 'undefined' || tid < 1) {
            _log_err('API release needs threadId argument')
        } else {
...
rpc.exports = {
    api: function(tid, api_funct, args) {
        if (DEBUG) {
            _log('[' + tid + '] RPC-API: ' + api_funct + ' | args: ' + args + ' (' + Process.getCurrentThreadId());
        }
...
var ELF_File = (function () {
    ...
    function ELF_File(path) {
        ...
        fs.fgets(buf, 64, _file);
        this.header = new ELF_Header(buf);

        if(this.header.e_ident[0] != 0x7f || this.header.e_ident[1] != 0x45 || this.header.e_ident[2] != 0x4c || this.header.e_ident[3] != 0x46) {
            __log('no elf file');
            fs.fclose(_file);
            return null;
        }
        ...
        for(var i = 0; i < this.header.e_phnum; i++) {
            fs.fseek(_file, this.header.e_phoff + this.header.e_phentsize * i, 0);
            fs.fgets(buf, this.header.e_phentsize, _file);
            this.programheaders.push(new ELF_ProgamHeader(buf, this.is64bit));
        }

        //get strtable
        fs.fseek(_file, this.header.e_shoff + this.header.e_shentsize * this.header.e_shstrndx, 0);
        fs.fgets(buf, this.header.e_shentsize, _file);
        ...
        for(var i = 0; i < this.header.e_shnum; i++) {
            fs.fseek(_file, this.header.e_shoff + this.header.e_shentsize * i, 0);
            fs.fgets(buf, this.header.e_shentsize, _file);
