...
function Dwarf() {
    ...
    this._onHook = function (reason, p, context, hook, java_handle) {
        ...
        that['context'] = new Proxy(context, {
                    ...
                    try {
                        ...
                        for (var s in getDwarf().nativeOnLoads) {
                            if (w.indexOf(s) >= 0) {
                                ...
                                loggedSend("native_on_load_callback:::" + hook.module + ':::' + 0 + ':::' + Process.getCurrentThreadId());
            ...
            if (javaHelper._sdk >= 23) {
                ...
                if (phdr_tgds_ptr > 0 && do_dlopen_ptr > 0) {
                    ...
                    Interceptor.attach(phdr_tgds_ptr, function (args) {
                        if (hook !== null) {
                            loggedSend("native_on_load_callback:::" + hook.module + ':::' + args[2] + ':::' + Process.getCurrentThreadId());
                            getDwarf()._onHook(REASON_HOOK, this.context.pc, this.context, hook, null);
                            hook = null;
                        }
                    });
                    ...
                    Interceptor.attach(do_dlopen_ptr, function (args) {
                        try {
                            ...
                            loggedSend('native_on_load_module_loading:::' + Process.getCurrentThreadId() + ':::' + w);
                            for (var s in getDwarf().nativeOnLoads) {
                                if (w.indexOf(s) >= 0) {
                                    ...
                                    for (var s in getDwarf().nativeOnLoads) {
                                        if (w.indexOf(s) >= 0) {
                                            ...
                                            if (typeof hook !== 'undefined') {
                                                var base = Process.findModuleByName(w).base;
                                                loggedSend("native_on_load_callback:::" + hook.module + ':::' + base + ':::' + Process.getCurrentThreadId());
                                                getDwarf()._onHook(REASON_HOOK, this.context.pc, this.context, hook, null);                                                
                                            }
                                        }
                                    }
...
function DwarfApi() {
    ...
    this.enumerateExports = function(module) {
        module = Process.findModuleByName(module);
        if (module !== null) {
            return JSON.stringify(module.enumerateExports())
        }
        return {};
    };
    ...
    this.enumerateImports = function(module) {
        module = Process.findModuleByName(module);
        if (module !== null) {
            return JSON.stringify(module.enumerateImports())
        }
        return {};
    };
    ...
    this.enumerateModules = function() {
        return JSON.stringify(Process.enumerateModules());
    };
    ...
    this.enumerateSymbols = function(module) {
        module = Process.findModuleByName(module);
        if (module !== null) {
            return JSON.stringify(module.enumerateSymbols())
        }
        return {};
    };
    ...
    this.getInitArrayPtrs = function(path) {
        ...
        if(elffile != null) {
            var base = parseInt(Process.findModuleByName(path.substring(path.lastIndexOf('/')).replace('/', ''))['base'], 16);
        ...
        Java.performNow(function () {
            ...
            if (SPAWNED) {
                if (javaHelper._sdk >= 23) {
                    ...
                    javaHelper._hook_in_jvm('android.app.Application', 'onCreate', true, null, false, true);
                }
            }
        ...
        if (Object.keys(getDwarf().hook_contexts).length > 0) {
            ...
            if (typeof hc !== 'undefined') {
                ...
                while (hc.next_api !== null) {
                    Thread.sleep(0.5);
                    if (DEBUG) {
                        _log('[' + tid + '] RPC-API: ' + api_funct + ' waiting for other api to return result');
                    }
                }
...
var ELF_File = (function () {
    ...
    function ELF_File(path) {
        ...
        if (ptr(fs.fgets(buf, 64, _file)) == ptr(0)) {
            __log('failed to read');
            fs.close(_file);
            return null;
        }
        this.header = new ELF_Header(buf);
        ...
        for(var i = 0; i < this.header.e_phnum; i++) {
            if(fs.fseek(_file, this.header.e_phoff + this.header.e_phentsize * i, 0) !== 0) {
                __log('fseek failed');
                fs.close(_file);
                return null;
            }
            if(ptr(fs.fgets(buf, this.header.e_phentsize, _file)) == ptr(0)) {
                __log('failed to read');
                fs.close(_file);
                return null;
            }
            this.programheaders.push(new ELF_ProgamHeader(buf, this.is64bit));
        }
        ...
        if(ptr(fs.fgets(buf, this.header.e_shentsize, _file)) == ptr(0)) {
            __log('failed to read');
            fs.close(_file);
            return null;
        }
        var section = new ELF_SectionHeader(buf, this.is64bit);
        ...
        var buf2 = fs.allocateRw(section.sh_size);
        if (buf2.isNull()) {
            __log('error alloc');
            fs.fclose(_file);
            return null;
        }
        if(ptr(fs.fgets(buf2, section.sh_size, _file)) == ptr(0)) {
            __log('failed to read');
            fs.close(_file);
            return null;
        }
        ...
        for(var i = 0; i < this.header.e_shnum; i++) {
            if(fs.fseek(_file, this.header.e_shoff + this.header.e_shentsize * i, 0) !== 0) {
                __log('fseek failed');
                fs.close(_file);
                return null;
            }
            ...
            section = new ELF_SectionHeader(buf, this.is64bit);
            section.name = Memory.readCString(buf2.add(section.sh_name));
            if (section.name === '.init_array') {
                var buf3 = fs.allocateRw(section.sh_size);
                if(fs.fseek(_file, section.sh_offset, 0) !== 0) {
                    __log('fseek failed');
                    fs.close(_file);
                    return null;
                }
                if(ptr(fs.fgets(buf3, section.sh_size, _file)) == ptr(0)) {
                    __log('failed to read');
                    fs.close(_file);
                    return null;
                }
                ...
                for(var a = 0; a < section.sh_size; a+=size) {
                    if(this.is64bit) {
                        section.data.push(Memory.readU64(buf3.add(a)).toNumber());
                    } else {
                        section.data.push(Memory.readU32(buf3.add(a)));
                    }
                }
            }
