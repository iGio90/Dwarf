...
function Dwarf() {
    ...
    this.start = function () {
        ...
        if (Process.platform === 'windows') {
            ...
            if (phdr_tgds_ptr > 0 && do_dlopen_ptr > 0) {
                ...
                Interceptor.attach(do_dlopen_ptr, function (args) {
                    try {
                        ...
                        loggedSend('onload_module_loading:::' + Process.getCurrentThreadId() + ':::' + w);
                        for (var s in getDwarf().onLoads) {
                            if (w.indexOf(s) >= 0) {
            ...
            javaHelper.hook('java.lang.ClassLoader',
                'loadClass', false,function(args) {
                    if (args.length === 2) {
                        loggedSend('class_loader_loading_class:::' + Process.getCurrentThreadId() +
                            ':::' + args['0']);
                    }
                }, false, true);
        }
...
function DwarfApi() {
    ...
    this.deleteHook = function(key) {
        ...
        if (typeof hook === 'undefined') {
            if (typeof getDwarf().onLoads[key] !== 'undefined') {
                loggedSend('hook_deleted:::onload:::' + key);
                delete getDwarf().onLoads[key];
            } else {
...
rpc.exports = {
    ...
    onloads: function() {
        return JSON.stringify(getDwarf().onLoads);
    },
    watchers: function() {
